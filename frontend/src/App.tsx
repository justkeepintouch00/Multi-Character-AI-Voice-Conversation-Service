...
function ScenarioFlowEditor({ mode, sceneIndex, characterName, branches, branchCounts, selectedId, sceneOptions, turns, onSelectScene, onSelect, onChange, onAddBranch, onRemoveBranch, onRemoveScene, onAddScene, onUpdateTurn }: {
  mode: Mode
  sceneIndex: number
  ...
  const [draggedEdge, setDraggedEdge] = useState<{ branchId: string; x: number; y: number } | null>(null)
  ...
  const activeTurn = turns[sceneIndex] ?? turns[0]
  const canvasWidth = 1100
  const graphHeight = Math.max(660, Math.max(branches.length, turns.length + 1) * 118 + 88)
  const sourceY = graphHeight / 2
  const branchY = (index: number) => 58 + index * 118
  const graphKey = `${mode}-scene-${sceneIndex}`
  const positionFor = (key: string, x: number, y: number) => nodePositions[`${graphKey}-${key}`] ?? { x, y }
  const startPosition = positionFor('start', 20, sourceY - 69)
  const classifierPosition = positionFor('classifier', 252, sourceY - 69)
  const branchPositions = branches.map((branch, index) => positionFor(branch.id, 492, branchY(index)))
  const scenePositions = turns.map((_, index) => positionFor(`scene-${index}`, 840, 42 + index * 118))
  const terminalPosition = positionFor('terminal', 840, 42 + turns.length * 118)
  const selectedBranchIndex = branches.findIndex((branch) => branch.id === selected?.id)
  const selectedBranchPosition = selectedBranchIndex >= 0 ? branchPositions[selectedBranchIndex] : null
  ...
  const detachOrStartConnection = (branch: FlowBranchDraft) => {
    onSelect(branch.id)
    if (branch.nextScene) onChange(branch.id, { nextScene: '' })
    setConnectingBranchId(branch.id)
  }
  const connectToScene = (targetIndex: number) => {
    if (!connectingBranchId) return
    const turn = turns[targetIndex]
    onChange(connectingBranchId, { nextScene: `장면 ${targetIndex + 1} · ${turn.situation.slice(0, 24)}` })
    setConnectingBranchId(null)
  }
  const beginEdgeDrag = (event: React.PointerEvent<SVGPathElement>, branch: FlowBranchDraft, position: { x: number; y: number }) => {
    event.stopPropagation()
    event.currentTarget.setPointerCapture(event.pointerId)
    setDraggedEdge({ branchId: branch.id, x: position.x + 232, y: position.y + 45 })
  }
  const moveEdgeDrag = (event: React.PointerEvent<SVGPathElement>) => {
    if (!draggedEdge) return
    const bounds = event.currentTarget.ownerSVGElement?.getBoundingClientRect()
    if (!bounds) return
    setDraggedEdge({ ...draggedEdge, x: (event.clientX - bounds.left) / zoom, y: (event.clientY - bounds.top) / zoom })
  }
  const finishEdgeDrag = (_event: React.PointerEvent<SVGPathElement>) => {
    if (!draggedEdge) return
    const target = turns.map((_, index) => scenePositions[index]).findIndex((position) => Math.hypot(position.x - draggedEdge.x, position.y + 49 - draggedEdge.y) < 90)
    if (target >= 0) connectToScene(target)
    else setDraggedEdge(null)
  }
  const requestRemoveBranch = (branch: FlowBranchDraft) => {
    if (window.confirm(`'${branch.label}' 반응 분기를 정말 삭제하시겠습니까?`)) onRemoveBranch(branch.id)
  }
  ...
  return <section className="builder-block flow-node-builder">
    ...
    <div className="flow-scene-tabs"><span>분기를 편집할 장면</span><div>{turns.map((turn, index) => <button type="button" key={`scene-tab-${index}`} className={sceneIndex === index ? 'active' : ''} onClick={() => onSelectScene(index)}><strong>장면 {index + 1}</strong><small>{branchCounts[index] ?? 0}개 분기 · {turn.background}</small></button>)}</div></div>
    <div className="flow-control-bar"><span>{connectingBranchId ? '연결할 장면에 선을 놓으세요.' : '노드 드래그·화면 이동·확대/축소 · 연결선을 직접 드래그해 재연결'}</span><div><button type="button" className="flow-add-branch-button" onClick={onAddBranch} disabled={branches.length >= 6}>＋ 반응 분기</button><button type="button" aria-label="축소" onClick={() => changeZoom(zoom - 0.1)}>−</button><strong>{Math.round(zoom * 100)}%</strong><button type="button" aria-label="확대" onClick={() => changeZoom(zoom + 0.1)}>＋</button><button type="button" className="reset-view" onClick={resetViewport}>화면 맞춤</button><button type="button" className="reset-view" onClick={resetNodeLayout}>배치 초기화</button></div></div>
    <div className={`flow-canvas-scroll ${isPanning ? 'is-panning' : ''} ${connectingBranchId ? 'is-connecting' : ''}`} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onWheel={(event) => { event.preventDefault(); changeZoom(zoom + (event.deltaY < 0 ? 0.08 : -0.08)) }}><div className={`flow-canvas mode-flow-${mode.toLowerCase()}`} style={{ width: canvasWidth, minWidth: canvasWidth, height: graphHeight, transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})` }}>
      <svg className="flow-connectors" viewBox={`0 0 ${canvasWidth} ${graphHeight}`} preserveAspectRatio="none" aria-hidden="true">
        ...
        {branches.map((branch, index) => {
          const branchPosition = branchPositions[index]
          ...
          return <g key={branch.id}><path className={`flow-line branch-line ${branch.id === selected?.id ? 'selected' : ''}`} d={connectorPath(classifierPosition.x + 190, classifierPosition.y + 69, branchPosition.x, branchPosition.y + 45)} />{branch.nextScene && <path className={`flow-line merge-line edge-draggable ${branch.id === selected?.id ? 'selected' : ''}`} d={connectorPath(branchPosition.x + 232, branchPosition.y + 45, draggedEdge?.branchId === branch.id ? draggedEdge.x : target.x, draggedEdge?.branchId === branch.id ? draggedEdge.y : target.y + 49)} onPointerDown={(event) => beginEdgeDrag(event, branch, branchPosition)} onPointerMove={moveEdgeDrag} onPointerUp={finishEdgeDrag} onPointerCancel={() => setDraggedEdge(null)} />}</g>
        })}
      </svg>
      <button type="button" className="flow-node scene-node start-node selected" style={{ left: startPosition.x, top: startPosition.y }} onPointerDown={(event) => handleNodePointerDown(event, 'start', startPosition, 182, 138)} onPointerMove={handleNodePointerMove} onPointerUp={handleNodePointerUp} onPointerCancel={handleNodePointerUp} onClick={() => onSelectScene(sceneIndex)}><span>현재 선택 장면</span><strong>{activeTurn?.situation ?? '장면 내용을 입력하세요.'}</strong><small>{characterName}의 대사와 화면 상황</small><i>{sceneIndex + 1}</i></button>
      <button type="button" className="flow-node classifier-node" style={{ left: classifierPosition.x, top: classifierPosition.y }} onPointerDown={(event) => handleNodePointerDown(event, 'classifier', classifierPosition, 190, 138)} onPointerMove={handleNodePointerMove} onPointerUp={handleNodePointerUp} onPointerCancel={handleNodePointerUp}><span>사용자 발화 분석</span><strong>의도·이유·맥락 분류</strong><small>LLM 분류 후 일치하는 경로 1개 실행</small><i>AI</i></button>
      {branches.map((branch, index) => <div key={branch.id}><button type="button" className={`flow-node branch-node ${branch.id === selected?.id ? 'selected' : ''} ${branch.fallback ? 'fallback' : ''}`} style={{ left: branchPositions[index].x, top: branchPositions[index].y }} onPointerDown={(event) => handleNodePointerDown(event, branch.id, branchPositions[index], 232, 90)} onPointerMove={handleNodePointerMove} onPointerUp={handleNodePointerUp} onPointerCancel={handleNodePointerUp} onClick={() => onSelect(branch.id)}>{branch.id === selected?.id && <em className="flow-editing-badge">편집 중</em>}<span>{branch.fallback ? '기본 경로' : '반응 조건'}</span><strong>{branch.label}</strong><small>{branch.reactionTone}</small><div><b className={branch.affinity >= 0 ? 'positive' : 'negative'}>호감 {deltaLabel(branch.affinity)}</b><b className={branch.trust >= 0 ? 'positive' : 'negative'}>신뢰 {deltaLabel(branch.trust)}</b></div></button><button type="button" className="node-delete-button" style={{ left: branchPositions[index].x + 201, top: branchPositions[index].y + 7 }} onClick={() => requestRemoveBranch(branch)} aria-label={`${branch.label} 삭제`} title="노드 삭제">×</button></div>)}
      {branches.map((branch, index) => <button type="button" key={`port-${branch.id}`} className={`flow-port branch-output-port ${connectingBranchId === branch.id ? 'active' : ''}`} style={{ left: branchPositions[index].x + 222, top: branchPositions[index].y + 36 }} onClick={() => detachOrStartConnection(branch)} aria-label={`${branch.label} 연결 ${branch.nextScene ? '분리' : '시작'}`} title={branch.nextScene ? '현재 선 분리 후 다시 연결' : '장면 연결 시작'} />)}
      {turns.map((turn, index) => <div key={`destination-scene-${index}`}><button type="button" className={`flow-node scene-node destination-scene-node ${sceneIndex === index ? 'selected' : ''}`} style={{ left: scenePositions[index].x, top: scenePositions[index].y }} onPointerDown={(event) => handleNodePointerDown(event, `scene-${index}`, scenePositions[index], 210, 98)} onPointerMove={handleNodePointerMove} onPointerUp={handleNodePointerUp} onPointerCancel={handleNodePointerUp} onClick={() => onSelectScene(index)}><span>장면 {index + 1}</span><strong>{turn.situation}</strong><small>{turn.background} · 선택하여 편집</small><i>{index + 1}</i></button><button type="button" className={`flow-port scene-input-port ${connectingBranchId ? 'ready' : ''}`} style={{ left: scenePositions[index].x - 7, top: scenePositions[index].y + 40 }} onClick={() => connectToScene(index)} disabled={!connectingBranchId} aria-label={`장면 ${index + 1}에 연결`} title="선택한 분기를 이 장면에 연결" /><button type="button" className="node-delete-button" style={{ left: scenePositions[index].x + 179, top: scenePositions[index].y + 7 }} onClick={() => requestRemoveScene(index)} aria-label={`장면 ${index + 1} 삭제`} title="노드 삭제">×</button></div>)}
      {branches.some((branch) => branch.nextScene && destinationIndex(branch.nextScene) < 0) && <button type="button" className="flow-node terminal-node" style={{ left: terminalPosition.x, top: terminalPosition.y }} onPointerDown={(event) => handleNodePointerDown(event, 'terminal', terminalPosition, 210, 98)} onPointerMove={handleNodePointerMove} onPointerUp={handleNodePointerUp} onPointerCancel={handleNodePointerUp}><span>{mode === 'C' ? '자유 대화' : '종료 흐름'}</span><strong>{mode === 'C' ? '평가 없이 대화 계속' : '결말 판정으로 이동'}</strong><small>장면이 아닌 시스템 목적지</small><i>✓</i></button>}
      <button type="button" className="flow-inline-add add-reaction-node" style={{ left: classifierPosition.x + 36, top: classifierPosition.y + 148 }} onClick={onAddBranch} disabled={branches.length >= 6}>＋ 반응 분기</button>
      {selectedBranchPosition && <button type="button" className="flow-inline-add add-next-scene-node" style={{ left: selectedBranchPosition.x + 48, top: selectedBranchPosition.y + 99 }} onClick={onAddScene}>＋ 이 분기 다음 장면</button>}
    </div></div>
...
function ScenarioBuilder({ onBack, onSaved }: { onBack: () => void; onSaved: () => void }) {
  ...
  const addFlowBranch = () => {
    if (activeFlowBranches.length >= 6) return
    const id = `${mode.toLowerCase()}-branch-${Date.now()}`
    const newBranch: FlowBranchDraft = { id, label: '새 반응 분기', responseType: '명확한 선택', condition: '이 분기로 보낼 사용자 발화의 의미와 행동 조건을 작성하세요.', reactionTone: '성격에 맞게 자연스럽게', reactionGuide: '캐릭터 성격을 유지하면서 사용자의 핵심 의도에 반응하고 다음 장면으로 자연스럽게 연결한다.', affinity: 0, trust: 0, boundary: 0, nextScene: draft.turns[activeFlowSceneIndex + 1] ? `장면 ${activeFlowSceneIndex + 2}` : mode === 'C' ? '자유 대화 · 계속' : '결말 판정', fallback: false }
    const fallback = activeFlowBranches.find((branch) => branch.fallback)
    const nextBranches = [...activeFlowBranches.filter((branch) => !branch.fallback), newBranch, ...(fallback ? [fallback] : [])]
    setFlowBranches((current) => ({ ...current, [mode]: { ...current[mode], [activeFlowSceneIndex]: nextBranches } }))
    setSelectedFlowBranchIds((current) => ({ ...current, [mode]: id }))
  }
  ...
  const addFlowScene = () => {
    const sceneIndex = draft.turns.length
    const sourceSceneIndex = activeFlowSceneIndex
    const sourceBranchId = selectedFlowBranchIds[mode]
    const linkedSceneName = `장면 ${sceneIndex + 1} · 새 장면`
    const nextTurn: TurnDraft = { situation: '새 장면을 설명해 주세요.', line: '캐릭터가 먼저 건넬 말을 작성해 주세요.', userGoal: '사용자가 이 장면에서 해야 할 행동을 작성해 주세요.', background: draft.background, backgroundImage: draft.coverImage || undefined }
    const nextBranches = createDefaultSceneBranches(mode, sceneIndex, sceneIndex + 1)
    updateDraft({ turns: [...draft.turns, nextTurn] })
    setFlowBranches((current) => ({
      ...current,
      [mode]: {
        ...current[mode],
        [sourceSceneIndex]: current[mode][sourceSceneIndex].map((branch) => branch.id === sourceBranchId ? { ...branch, nextScene: linkedSceneName } : branch),
        [sceneIndex]: nextBranches,
      },
    }))
  }
...
function SnackSelectionScenario({ scenario, onExit, onFinish }: { scenario: Scenario; onExit: () => void; onFinish: (result: ResultData) => void }) {
  ...
  return <div className="runner-page snack-scenario-page">
    ...
    <main className="scene-stage stage-snack-art"><div className="scene-conversation-layer"><div className="scene-narration">{round.narration}</div><article className="live-caption-card haru-caption"><PersonAvatar name="하루" accent="violet" image={characterPortraits.하루} /><div><span>하루</span><p>{activeLine}</p></div></article><article className={`live-caption-card user-caption ${transcript ? 'has-speech' : ''}`}><PersonAvatar name="나" accent="slate" /><div><span>사용자 · 실시간 자막</span><p>{transcript ? `“${transcript}”` : voiceState === 'listening' ? '말하는 내용을 듣고 있어요…' : voiceState === 'denied' ? '마이크 권한이 필요합니다.' : '음성 인식을 준비하고 있어요…'}</p></div><b aria-label="음성 자동 인식 중">{voiceState === 'listening' && !reply ? 'LIVE' : reply ? '완료' : '대기'}</b></article></div></main>
