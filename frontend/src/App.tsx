...
const initialScenarios: Scenario[] = [
  ...
  { id: 'first-talk', mode: 'C', title: '공방이 문을 닫은 뒤', summary: '말할 준비가 될 때까지 루미의 일상을 따라가며 천천히 대화를 시작하는 이야기', characterNames: ['루미'], characterIds: ['character_a'], duration: '자유 대화', published: true, plays: 203, opening: '마침 나도 공방 문을 닫으려던 참이었어. 처음부터 말하려니 조금 어색할 수 있겠다. 말하기 힘들면 내 이야기만 들어도 괜찮아.' },
...
const initialFlowBranches: Record<Mode, FlowBranchDraft[]> = {
  A: [
    ...
    { id: 'a-clear-only', label: '선택만 말함', responseType: '명확한 선택', condition: '선택은 했지만 이유가 없거나 매우 짧음', reactionTone: '가볍게 재질문', reactionGuide: '선택을 긍정한 뒤 이유를 한 번만 묻는다. 압박하거나 정답을 암시하지 않는다.', affinity: 1, trust: 1, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'a-avoid', label: '결정을 미룸', responseType: '회피 또는 책임 전가', condition: '“아무거나”, “네가 골라”처럼 선택을 상대에게 넘김', reactionTone: '장난스럽지만 단호하게', reactionGuide: '캐릭터 성격을 유지하면서 선택권을 사용자에게 다시 돌려준다.', affinity: -2, trust: -1, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'a-question', label: '캐릭터에게 질문', responseType: '상대에게 질문', condition: '선택 전에 캐릭터의 취향이나 이유를 질문함', reactionTone: '솔직하게 답변', reactionGuide: '자신의 취향을 짧게 답한 뒤 사용자의 선택을 다시 요청한다.', affinity: 1, trust: 2, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'a-fallback', label: '그 밖의 응답', responseType: '분류되지 않은 응답', condition: '위 조건에 해당하지 않거나 맥락이 불명확함', reactionTone: '차분하게 맥락 확인', reactionGuide: '사용자의 말을 추측해서 단정하지 말고 현재 선택지를 다시 짧게 안내한다.', affinity: 0, trust: 0, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: true },
  ],
  B: [
    { id: 'b-coordinate', label: '조건까지 조율', responseType: '의견 조율과 조건 확인', condition: '여러 의견과 시간·음식 제한을 함께 반영함', reactionTone: '신뢰하며 동의', reactionGuide: '제안이 실제로 가능한지 확인하고 캐릭터가 동의하는 이유를 말한다.', affinity: 3, trust: 5, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'b-majority', label: '다수 의견만 반영', responseType: '빠른 다수결', condition: '다수의 취향은 반영했지만 제한 조건을 확인하지 않음', reactionTone: '결정은 인정하고 우려 표현', reactionGuide: '결정 속도는 인정하되 빠진 제한 조건을 구체적으로 한 번 확인한다.', affinity: 0, trust: -1, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    ...
    { id: 'b-fallback', label: '그 밖의 응답', responseType: '분류되지 않은 응답', condition: '의견 조율과 관련 없는 답변이 들어옴', reactionTone: '현재 조건 다시 정리', reactionGuide: '이미 나온 의견과 제한 조건만 간단히 다시 말하고 제안을 요청한다.', affinity: 0, trust: 0, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: true },
  ],
  C: [
    { id: 'c-user-story', label: '내 이야기부터 시작', responseType: '사용자 자기 이야기', condition: '사용자가 자신의 일상이나 감정을 먼저 이야기함', reactionTone: '서두르지 않고 경청', reactionGuide: '해결책을 바로 제시하지 않고 사용자의 표현을 짧게 반영한 뒤 열린 질문을 하나만 한다.', affinity: 2, trust: 4, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'c-character-story', label: '캐릭터 이야기 듣기', responseType: '캐릭터에게 이야기 요청', condition: '사용자가 말하기보다 캐릭터의 일상을 먼저 듣고 싶어 함', reactionTone: '일상을 가볍게 공유', reactionGuide: '부담이 낮은 일상 이야기를 짧게 들려주고 사용자가 원할 때만 질문한다.', affinity: 1, trust: 2, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'c-silence', label: '침묵 또는 짧은 응답', responseType: '대화 진입이 어려움', condition: '침묵이 길거나 한두 단어만 말함', reactionTone: '편안하게 기다림', reactionGuide: '재촉하지 않고 선택지를 두 개만 제시하거나 자신의 이야기를 조금 이어간다.', affinity: 0, trust: 1, boundary: 1, nextScene: '장면 2 · 다음 장면', fallback: false },
    { id: 'c-fallback', label: '그 밖의 응답', responseType: '자유 응답', condition: '정해진 진입 유형으로 분류되지 않음', reactionTone: '성격에 맞게 자연스럽게', reactionGuide: '캐릭터 프로필과 최근 대화 맥락을 우선하며 평가나 결말 판정을 하지 않는다.', affinity: 0, trust: 0, boundary: 0, nextScene: '장면 2 · 다음 장면', fallback: true },
  ],
}
...
const createDefaultSceneBranches = (mode: Mode, sceneIndex: number, turnCount: number): FlowBranchDraft[] => {
  const nextScene = sceneIndex + 1 < turnCount ? `장면 ${sceneIndex + 2}` : ''
  ...
  return [
    ...
    { id: `${prefix}-partial`, label: '일부만 답함', responseType: '명확한 선택', condition: '사용자가 장면의 질문에는 답했지만 필요한 이유나 조건 일부를 빠뜨림', reactionTone: '가볍게 재질문', reactionGuide: '이미 답한 부분은 반복해서 요구하지 말고 빠진 정보 한 가지만 짧게 확인한다.', affinity: 0, trust: 1, boundary: 0, nextScene, fallback: false },
    { id: `${prefix}-avoid`, label: '대답을 피하거나 넘김', responseType: '회피 또는 책임 전가', condition: '사용자가 결정을 미루거나 답변 책임을 캐릭터에게 넘김', reactionTone: '차분하게 맥락 확인', reactionGuide: '비난하지 않고 선택권을 사용자에게 돌려주며 현재 장면의 선택지를 다시 간단히 안내한다.', affinity: -1, trust: -1, boundary: 0, nextScene, fallback: false },
    { id: `${prefix}-fallback`, label: '그 밖의 응답', responseType: '분류되지 않은 응답', condition: '위 조건과 일치하지 않거나 현재 장면의 맥락이 불명확함', reactionTone: '성격에 맞게 자연스럽게', reactionGuide: '사용자 의도를 임의로 단정하지 말고 현재 상황을 짧게 확인한 뒤 다시 말할 기회를 준다.', affinity: 0, trust: 0, boundary: 0, nextScene, fallback: true },
  ]
}
...
function ScenarioFlowEditor({ mode, sceneIndex, characterName, branches, branchCounts, selectedId, turns, endings, onSelectScene, onChange, onRemoveBranch, onRemoveScene, onUpdateTurn, allFlowBranches, onGraphSelect, onGraphChange, onGraphAddBranch, onGraphRemoveBranch }: {
  ...
  const canvasWidth = Math.max(1560, 190 + turns.length * 760 + Math.max(1, endings.length) * 250)
  const graphHeight = Math.max(760, Math.max(...turns.map((_, index) => (allFlowBranches[index]?.length ?? 0) * 120 + 150)) + 90)
  const sourceY = Math.max(380, Math.round(graphHeight * .56))
  const graphKey = `${mode}-full-flow`
  const positionFor = (key: string, x: number, y: number) => nodePositions[`${graphKey}-${key}`] ?? { x, y }
  const startPosition = positionFor('start', 18, sourceY - 69)
  const sceneLanePositions = turns.map((_, index) => ({ x: 220 + index * 650, y: sourceY - 69 }))
  const scenePositions = turns.map((_, index) => positionFor(`scene-${index}`, sceneLanePositions[index].x, sceneLanePositions[index].y))
  const graphBranchPosition = (currentSceneIndex: number, branch: FlowBranchDraft, branchIndex: number) => {
    const branchCount = allFlowBranches[currentSceneIndex]?.length ?? 0
    const y = sourceY - ((Math.max(branchCount, 1) - 1) * 56) - 45 + branchIndex * 112
    return positionFor(`graph-${currentSceneIndex}-${branch.id}`, sceneLanePositions[currentSceneIndex].x + 250, y)
  }
  const endingPositions = endings.map((_, index) => positionFor(`ending-${index}`, sceneLanePositions[turns.length - 1].x + 650, sourceY - ((Math.max(endings.length, 1) - 1) * 56) - 49 + index * 112))
  ...
  const endingIndex = (nextScene: string) => {
    if (nextScene === '결말 판정') return endings.length ? 0 : -1
    const name = nextScene.replace(/^결말\s*:\s*/, '')
    return nextScene.startsWith('결말') ? endings.findIndex((ending) => ending.name === name) : -1
  }
  const connectBranch = (sourceSceneIndex: number, branchId: string, x: number, y: number) => {
    const sceneTarget = scenePositions.findIndex((target) => Math.hypot(target.x - x, target.y + 69 - y) < 92)
    if (sceneTarget >= 0) {
      onGraphChange(sourceSceneIndex, branchId, { nextScene: `장면 ${sceneTarget + 1} · ${turns[sceneTarget].situation.slice(0, 24)}` })
      return
    }
    const endingTarget = endingPositions.findIndex((target) => Math.hypot(target.x - x, target.y + 49 - y) < 92)
    if (endingTarget >= 0) onGraphChange(sourceSceneIndex, branchId, { nextScene: `결말: ${endings[endingTarget].name}` })
    else onGraphChange(sourceSceneIndex, branchId, { nextScene: '' })
  }
  ...
  return <section className="builder-block flow-node-builder">
    <div className="block-heading"><div><h2>3. 대화 분기 설계</h2><span>한 캔버스에서 모든 장면, 발화 분석, 반응, 결말의 연결을 직접 설계합니다.</span></div><div className="flow-heading-actions"><span className="flow-count-chip">장면 {turns.length}개</span><span className="flow-count-chip">전체 반응 {Object.values(allFlowBranches).flat().length}개</span></div></div>
    ...
    <div className={`flow-canvas-scroll ${isPanning ? 'is-panning' : ''} ${draggedEdge ? 'is-connecting' : ''}`} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onWheel={(event) => { event.preventDefault(); changeZoom(zoom + (event.deltaY < 0 ? 0.08 : -0.08)) }}><div className={`flow-canvas mode-flow-${mode.toLowerCase()}`} style={{ width: canvasWidth, minWidth: canvasWidth, height: graphHeight, transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})` }}>
      <svg className="flow-connectors" viewBox={`0 0 ${canvasWidth} ${graphHeight}`} preserveAspectRatio="none" aria-hidden="true">
        ...
        {turns.map((_, currentSceneIndex) => (allFlowBranches[currentSceneIndex] ?? []).map((branch, branchIndex) => {
          ...
          const resolvedEndingIndex = endingIndex(branch.nextScene)
          const target = targetIndex >= 0 ? scenePositions[targetIndex] : resolvedEndingIndex >= 0 ? endingPositions[resolvedEndingIndex] : null
          const dragging = draggedEdge?.branchId === branch.id && draggedEdge.sceneIndex === currentSceneIndex
          return <g key={`edge-${currentSceneIndex}-${branch.id}`}><path className={`flow-line branch-line ${currentSceneIndex === sceneIndex && branch.id === selected?.id ? 'selected' : ''}`} d={connectorPath(scenePositions[currentSceneIndex].x + 182, scenePositions[currentSceneIndex].y + 69, branchPosition.x, branchPosition.y + 45)} />{(target || dragging) && <path className="flow-line merge-line" d={connectorPath(branchPosition.x + 232, branchPosition.y + 45, dragging ? draggedEdge.x : target!.x, dragging ? draggedEdge.y : target!.y + (targetIndex >= 0 ? 69 : 49))} />}</g>
        }))}
      </svg>
...
function SnackSelectionScenario({ scenario, onExit, onFinish }: { scenario: Scenario; onExit: () => void; onFinish: (result: ResultData) => void }) {
  ...
  return <div className="runner-page snack-scenario-page">
    ...
    <main className="scene-stage stage-snack-art"><div className="scene-conversation-layer"><div className="scene-narration">{round.narration}</div><article className="live-caption-card haru-caption"><PersonAvatar name="하루" accent="violet" image={characterPortraits.하루} /><div><span>하루</span><p>{activeLine}</p></div></article></div></main>
    <aside className="interaction-dock snack-interaction-dock"><div className="turn-guidance"><span>{reply ? '응답 확인' : '이번 턴'}</span><strong>{reply ? (reply.advance ? '하루의 반응을 확인하고 다음 비교로 이동합니다.' : '같은 질문에 다시 답합니다.') : round.goal}</strong></div>{!reply && <AlwaysListening transcript={transcript} state={voiceState} />}<div className="text-response">{reply ? <button type="button" className="primary-button continue-button" onClick={continueScenario}>{reply.advance ? '다음 비교로 이동' : '다시 답하기'} →</button> : <form onSubmit={(event) => { event.preventDefault(); submitAnswer(typed) }}><label><span>키보드 대체 입력</span><input value={typed} onChange={(event) => setTyped(event.target.value)} placeholder="선택과 이유를 한 문장으로 입력하세요" /></label><button type="submit" disabled={!typed.trim()}>전송</button></form>}</div></aside>
  </div>
}
...
function composePersonaFromFields(form: Pick<Character, 'traits' | 'speech' | 'length' | 'relation'>): string {
  const traitsText = form.traits.length ? form.traits.join(', ') : '자연스러운'
